# concept

##  layers
domain: a -> b (реактивный)
application: A -> B /modules (A = a + infra; B = b + infra)
cmd: (A -> B) * Time (процессы)

domain:
  - работает только с домееными value типам
application:
  - работает с низкоуровневыми типами
  - преобразует их в доменные типы
cmd:
  - работает с пользовательским вводом (строки итд)
  - преобразует их в низкоуровневые типы


# Structure

+ build
+ config
+ deployment
- cmd
- subsystems
  + account
  + crm
  - authentication
    - event_handler.go
    - cmd
      - http_server
    - internal
      - system.go //  di container
      - register_user.go
      - internal // pure domain
        - models
          user.go
        - events
          user_registered.go
        - event_handler.go // connects



    - pgk
      - postgres
      - telegram

+ infrastructure

---
# Subsystems

## authorization
  - http_api
    authenticate (email + password)
    recover_password

  - cli_app
    register_idetnity

## accounts
  - organisation { id, name }
  api:
   doAddOrganization


## yclients
  model:
  - salon { organization_id, salon_id}
  - client { salon_id }
  - visit { client_id, }
  api:
    getClientsAddedAfter()
    getClentsRemovedAfter()
    doExcludeClientFromSync()
    doIncludeClientToSync()

## crm

 ## users
  - registration
  - password recovery
  - login

 Account
 User
    accountId
    fullname
    password
    phoneNumber
    type: ACCOUNT_MANAGER | USER

# clients
  - period calculation
  - visits history
  - notifications

  Client
      accountId
      fullname

  Visit



# ycapp
  register user
  receive all clients
  keep up client history

YCUser
  name
  accountId

YCSalon
    ycSalonId
    ycUserId
    name

YCSalonClient
    ycSalonId
    clientId


YCEvent



### Folders

ycapp
  cmd
    yc_app
  internal
    core
    application
      services
      http_client
  api
mainapp
  cmd
  internal
    core
    application

# ---

handlers
systems/modules
yclients
auth


```
registration
  register_user_by_yclients_app()

yclientsapp
  <- registration_api?
  register_user(id, password, salon_id)
  [ - registration.create_account()
    - registration.register_user()
    - yclientsapp.add_salon()
  ]
```

# handler

1. parse/validate params
2. authenticate user
3. execute app logic

```go
	func handler(request) error {
	  params, err = cast(request.params, schema?)

	  tx = txManager.NewTransaction(ctx)

	  usersApp = NewUsersApp()
	  regYclientsApp = NewRegisterYClientsAppFeature(tx)

	  appId, err := regYclientApp.Call(params)
	  err := usersApp.RegisterUser(params + appId)

	  return nil
	}
```

# application

salonsRepo = NewYclients
salonClientsRepo = NewYclients
